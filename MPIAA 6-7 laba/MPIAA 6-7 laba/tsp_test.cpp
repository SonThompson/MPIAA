...
TEST_CASE("[TSP] Empty graph", "[tsp]") {
    ...
    CHECK(tsp_full_enumeration(g, 0) == vector<int> {});
}
...
TEST_CASE("[TSP] One edge", "[tsp]") {
    ...
    CHECK(tsp_full_enumeration(g, 0) == vector<int> {0, 1});
}

TEST_CASE("[TSP] Three vertices, three edges", "[tsp]") {
    Graph g{ {0, 1, 2.5}, {0, 2, 0.5}, {1, 2, 1.0} };
    const auto result1 = tsp_bnb(g, 0);
    const auto result2 = tsp_full_enumeration(g, 0);
    const auto expected = vector<int>{ 0, 1, 2 };
    CHECK(min_dir(result1) == expected);
    CHECK(min_dir(result2) == expected);
}
...
/*TEST_CASE("[TSP] Unreachable vertex", "[tsp]") {
    ...
    CHECK(tsp_full_enumeration(g, 0) == vector<int> {});
    CHECK(tsp_full_enumeration(g, 3) == vector<int> {});
}
TEST_CASE("[TSP] No looped path", "[tsp]") {
    ...
    CHECK(tsp_full_enumeration(g, 0) == vector<int> {});
    CHECK(tsp_full_enumeration(g, 1) == vector<int> {});
}*/