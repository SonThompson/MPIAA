...
double LowerBound2(const Graph& g, vector<int> visited) {
    double result = 0;
    for (int i = 0; i < g.get_vertices().size(); ++i)
    {
        ...
        if (find(visited.begin(), visited.end(), i) == visited.end()) {
            ...
            sort(u.begin(), u.end(), [](const pair<int, double>& u1, const pair<int, double>& u2) {return u1.second < u2.second; });
            for (int j = 0; j < u.size(); ++j)
            {
                if (find(visited.begin(), visited.end(), u[j].first) == visited.end()) {
                    weights.push_back(u[j].second);
                }
            }
        }
        if (weights.size() == 1)
